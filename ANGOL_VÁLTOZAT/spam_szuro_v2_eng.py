...
# Streamlit felület
st.set_page_config(page_title="Spam-szűrő_ENG", layout="wide", initial_sidebar_state="expanded")
...
def plot_naive_bayes_distribution():
    ...
    spam_words = [word for word in data[data['Label'] == 1]['Text'].str.cat(sep=' ').split() if word not in stopwords.words('english')]
    ham_words = [word for word in data[data['Label'] == 0]['Text'].str.cat(sep=' ').split() if word not in stopwords.words('english')]
...
st.sidebar.markdown("Ez az alkalmazás spam e-mailek azonosítására szolgál a Naive Bayes algoritmus segítségével. Ez a verzió felismeri az eddigi e-maileket, és tanítani tudja magát a visszajelzések alapján. A modell csak <span style='color:red; font-weight:bold;'>angol</span> kifejezésekre van tanítva.", unsafe_allow_html=True)

st.sidebar.title("Példa e-mailek")
st.sidebar.write("Spam példa: 'Congratulations! You've won a gift card!'")
st.sidebar.write("Ham példa: 'Meeting tomorrow at 10 AM.'")
